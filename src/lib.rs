...
impl EventWrapper {
    ...
    pub fn new(n_channels: usize, waveform_len: usize) -> Self {
        ...
        let waveform_ptrs = waveform_ptrs_vec.into_boxed_slice();
        ...
        let c_event = CEvent {
            ...
            trigger_id: 0,
            event_size: 0,
            waveform: waveform_ptrs.as_ptr() as *mut *mut u16,
...
pub fn felib_readdata(handle: u64, data: &mut EventWrapper) -> Result<(), FELibError> {
    let res = unsafe {
        CAEN_FELib_ReadData(
            handle,
            100,
